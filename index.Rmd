...
Lorsque l'on combine les données en confondant les types cellulaires, on observe mieux certaines valeurs extrêmes. Etant donné que les données ont été précédemment manipulées lors de la normalisation, nous supposons que celles-ci ont également été filtré avant normalisation. Ainsi nous ne filtreront pas les données dans notre cas et prendrons l'ensemble de ces données à cette étape.
...
Cette représentation permet de distinguer différents types cellulaires répartis le long des deux dimensions de la représentation UMAP. Un groupe en particulier se détache des autres (6) et semble être différent. Malgré cela, les types cellulaires même regroupés semblent se distinguer. Afin que le traitement des données soit plus simple, nous avons décidé de réduire nos données en 4 clusters.
...
Nous pouvons ainsi observer sur l'ACP que parmis les 4 clusters sélectionnés, nous distinguons 3 types cellulaires qui se répartissent en 3 groupes (0, 1 et 2) le quatrième groupe (3) se confondant aux 3 autres. Lorsque l'on regarde cette représentation en format UMAP nous pouvons observer 4 groupes bien distincts. Les types cellulaires 0,1 et 2 sont très proches les uns des autres et le cluster 3 qui est excentré par rapport à ce groupe.
...
Ces gènes nous permettent ainsi de mettre en évidence à quel groupe immunitaire appartiennent chaque type cellulaire de notre UMAP. Cependant seuls, ils ne sont pas spécifiques à nos groupes et il faut que nous recherchions des marqueurs de ces gènes pour l'ensemble de notre population nous permettant de définir les types cellulaires spécifiques à nos groupes.
...
-CFD (Complement Factor D) : présent dans les macrophages, neutrophiles et cellules endothéliales.

-CXCL2 (C-X-C Motif Chemokine Ligand 2) : présent dans les Neutrophiles, monocytes, et macrophages.

-CCL19 (C-C Motif Chemokine Ligand 19) : présent dans Lymphocytes T, cellules dendritiques, et lymphocytes B

-LAD1 (Leukocyte Adhesion Deficiency 1) : présent dans les leucocytes

-PACSIN1 (Protein Kinase C and Casein Kinase Substrate In Neurons 1) : présent dans les macrophages, dendritiques.

-TCL1A (T-cell Leukemia 1A) : présent dans les cellules dendritique

-UBE2C (Ubiquitin-Conjugating Enzyme E2C) : non spécifique.

-CDC2 (Cell Division Cycle 2) : présent dans les cellules des lymphocytes T.
...
Ces résultats sont assez bien corrélés avec les résultats d'expression présents sur l'UMAP. Ainsi l'expression n'est pas forcément corrélée à un type cellulaire mais en lien avec les interaction des autres populations.
