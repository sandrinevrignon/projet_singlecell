...
```{r}
myeloid<-readRDS("C://Users//sandr//OneDrive//Bureau//Cours fac//M2//Bioanalyse, transcriptomique//Single cell//dossier//myeloid-cells.RDS")
myeloid[["percent.mt"]] <- PercentageFeatureSet(myeloid, pattern="^MT-")  
VlnPlot(myeloid, features=c("nFeature_RNA"), ncol=1)
VlnPlot(myeloid, features=c("nCount_RNA"), ncol=1)
VlnPlot(myeloid, features=c("percent.mt"), ncol=1)
```
...
```{r}
plot1 <- FeatureScatter(myeloid, feature1="nCount_RNA", feature2="percent.mt")
plot2 <- FeatureScatter(myeloid, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1+plot2
```
...
```{r}
DimPlot(myeloid, reduction = "umap", label = TRUE, pt.size = 0.5)
DimPlot(myeloid, reduction = "tsne", label = TRUE, pt.size = 0.5)
DimPlot(myeloid, reduction = "pca", label = TRUE, pt.size = 0.5)

```

## Identification des cluster et réduction à 4 clusters

```{r}

myeloid <- FindNeighbors(myeloid, dims = 1:10) 
myeloid <- FindClusters(myeloid, resolution = 0.075)

```

## Renommation des clusters

```{r}

# Renommer les clusters en "Cluster 1" à "Cluster 4"
new.cluster.ids <- paste("C",1:length(levels(myeloid))) # Génère les noms des clusters
...
# Visualiser avec les nouveaux noms
DimPlot(myeloid, reduction = "umap", label = TRUE, pt.size = 0.5)

# Identifier les gènes marqueurs pour les clusters renommés
myeloid.markers <- FindAllMarkers(myeloid, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)

# Visualiser les top marqueurs
top10 <- myeloid.markers %>%
  group_by(cluster) %>%
  top_n(n = 10, wt = avg_log2FC)

DoHeatmap(myeloid, features = top10$gene) + NoLegend()

# Visualiser les gènes d'intérêt (SPP1 et CXCL9)
FeaturePlot(myeloid, features = c("SPP1", "CXCL9"))

# Ajouter un ViolinPlot pour CXCL9 et SPP1
VlnPlot(myeloid, features = c("CXCL9", "SPP1"), group.by = "seurat_clusters", pt.size = 0)

# Installation et utilisation de SingleCellSignalR
...
counts <- as.matrix(GetAssayData(myeloid, layer = "counts"))
cluster.num <- as.numeric(Idents(myeloid))
...
Nous pouvons ainsi observer que les 4 groupes se détachent répartie de
